...
<body class="min-h-screen flex flex-col justify-between select-none" data-theme="default">
  ...
  <header class="p-3.5 bg-[#121520] border-b border-[#222738] sticky top-0 z-30 flex items-center justify-between shadow-lg">
    ...
    <div class="flex items-center gap-1.5">
      <button onclick="openVaniAIModal()" class="p-2 rounded-xl bg-gradient-to-r from-violet-600/40 to-indigo-600/40 border border-indigo-400/50 text-[11px] text-indigo-300 hover:text-white transition shadow" title="🤖 Ask Vani AI">
      ...
      <button onclick="navigateTo('dashboard')" class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#181d2c] border border-[#2b3348] text-[11px] text-gray-200 hover:border-indigo-500 transition">
        ...
        <span id="header-user-name" class="font-medium text-xs truncate max-w-[60px]">Devotee</span>
      </button>
    </div>
  </header>
    ...
    <div class="flex gap-2">
      <div class="relative flex-1">
        ...
        <input
          ...
          class="w-full bg-[#121520] border border-[#222738] rounded-xl pl-9 pr-14 py-2.5 text-xs text-gray-100 placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition"
        />
        <div class="absolute right-2.5 top-2 flex items-center gap-1">
          <button id="search-clear-btn" onclick="clearSearch()" class="text-gray-500 hover:text-white hidden text-xs p-1">
            <i class="fa-solid fa-xmark"></i>
          </button>
          <button onclick="startVoiceSearch()" id="voice-search-btn" class="text-indigo-400 hover:text-indigo-300 text-xs p-1" title="🎙️ Voice Search">
    ...
    <section id="view-dashboard" class="space-y-4 hidden">
      ...
      <div class="p-4 rounded-2xl bg-gradient-to-br from-[#161a28] to-[#111420] border border-[#262c3e] shadow-xl space-y-3">
        ...
        <div class="grid grid-cols-4 gap-1.5 pt-1 text-center">
          ...
          <div class="p-2 rounded-xl bg-[#121520] border border-[#222738]">
            <p id="stat-japa-rounds" class="text-xs font-bold text-emerald-400 font-mono">0</p>
            <p class="text-[8px] text-gray-400 uppercase">Japa Rounds</p>
          </div>
        </div>
      </div>
  ...
  <div id="full-player-modal" class="fixed inset-0 bg-[#0b0d13] z-50 p-5 flex flex-col justify-between hidden animate-slideUp">
    ...
    <div class="flex items-center justify-between border-b border-[#222738] pb-3">
      ...
      <div class="flex items-center gap-1.5">
        <button onclick="openVaniAIModal()" class="text-gray-400 hover:text-amber-400 p-1.5 text-xs" title="Ask Vani AI about this lecture">
          <i class="fa-solid fa-robot text-base text-amber-400"></i>
        </button>
  ...
  <!-- 🤖 Vani AI Assistant Modal Dialog -->
  <div id="vani-ai-modal" class="fixed inset-0 bg-black/85 z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-[#141824] border border-[#2a3248] rounded-2xl max-w-md w-full p-4 space-y-3 shadow-2xl animate-slideUp max-h-[85vh] flex flex-col">
      <div class="flex items-center justify-between border-b border-[#232838] pb-2">
        <h3 class="text-xs font-bold text-indigo-300 flex items-center gap-1.5">
          ...
          <span>Vani AI • Spiritual & Purport Companion</span>
        </h3>
        <button onclick="closeVaniAIModal()" class="text-gray-400 hover:text-white text-xs p-1"><i class="fa-solid fa-xmark"></i></button>
      </div>

      <!-- Quick Prompt Chips -->
      <div class="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none text-[10px]">
        <button onclick="askAIPreset('Explain the core purports of Bhagavad Gita 2.13 and the nature of the soul')" class="px-2.5 py-1 rounded-full bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 whitespace-nowrap hover:bg-indigo-600 hover:text-white transition">📖 BG 2.13 Soul</button>
        <button onclick="askAIPreset('How does Dr. Laxmidhar Behera explain Sankhya philosophy vs modern Quantum Physics?')" class="px-2.5 py-1 rounded-full bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 whitespace-nowrap hover:bg-indigo-600 hover:text-white transition">🔬 Sankhya & Quantum</button>
        <button onclick="askAIPreset('What are the 6 urges to control according to Nectar of Instruction Verse 1?')" class="px-2.5 py-1 rounded-full bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 whitespace-nowrap hover:bg-indigo-600 hover:text-white transition">🪔 NOI Verse 1</button>
      </div>

      <!-- AI Chat Message Log -->
      <div id="ai-chat-log" class="flex-1 overflow-y-auto space-y-2.5 text-xs p-3 rounded-xl bg-[#0d1017] border border-[#222738] min-h-[200px]">
        <div class="p-2.5 rounded-xl bg-[#161a28] border border-[#262f44] text-gray-200 leading-relaxed">
          <span class="font-bold text-amber-300 block pb-1">🙏 Hare Krishna!</span>
          I am your Vani AI assistant. Ask any question about Dr. Laxmidhar Behera Sir's lectures, Bhagavad Gita purports, or Sankhya philosophy!
        </div>
      </div>

      <!-- AI Input Bar -->
      <div class="flex gap-2">
        <input id="ai-user-input" type="text" placeholder="Ask a question about Gita, Sankhya, or Purports..." onkeydown="if(event.key==='Enter') sendAIMessage()" class="flex-1 bg-[#0d1017] border border-[#222738] rounded-xl px-3 py-2 text-xs text-gray-100 placeholder-gray-500 focus:outline-none focus:border-indigo-500" />
        <button onclick="sendAIMessage()" class="px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold flex items-center gap-1 shadow">
  ...
  <!-- 📿 Japa Mala Counter Modal Dialog -->
  <div id="japa-modal" class="fixed inset-0 bg-black/85 z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-[#141824] border border-[#2a3248] rounded-2xl max-w-sm w-full p-5 space-y-4 shadow-2xl animate-slideUp text-center">
      <div class="flex items-center justify-between border-b border-[#232838] pb-2 text-left">
        <h3 class="text-xs font-bold text-amber-300 flex items-center gap-1.5">
          <i class="fa-solid fa-dharmachakra"></i>
          <span>Japa Mala Sadhana Counter</span>
        </h3>
        <button onclick="closeJapaModal()" class="text-gray-400 hover:text-white text-xs p-1"><i class="fa-solid fa-xmark"></i></button>
      </div>

      <!-- Hare Krishna Mahamantra Header -->
      <div class="p-3 rounded-xl bg-[#0d1017] border border-amber-500/20 text-amber-200 font-serif text-xs leading-relaxed">
        हरे कृष्ण हरे कृष्ण कृष्ण कृष्ण हरे हरे ।<br/>
        हरे राम हरे राम राम राम हरे हरे ॥
      </div>

      <!-- Large Interactive Bead Tap Button -->
      <div class="py-2">
        <button onclick="tapJapaBead()" class="w-36 h-36 mx-auto rounded-full bg-gradient-to-tr from-amber-600 via-orange-500 to-amber-400 text-white flex flex-col items-center justify-center shadow-2xl shadow-amber-500/30 active:scale-95 transition cursor-pointer border-4 border-amber-300/40">
          <span id="japa-bead-count" class="text-3xl font-bold font-mono">0</span>
          <span class="text-[10px] uppercase tracking-wider font-bold text-amber-100">/ 108 Beads</span>
        </button>
      </div>

      <!-- Japa Stats -->
      <div class="grid grid-cols-2 gap-2 text-xs">
        <div class="p-2.5 rounded-xl bg-[#0e111a] border border-[#222738]">
          <p class="text-[10px] text-gray-400">Rounds Completed</p>
          <p id="japa-round-count" class="text-base font-bold text-emerald-400 font-mono">0 / 16</p>
        </div>
        <div class="p-2.5 rounded-xl bg-[#0e111a] border border-[#222738]">
          <p class="text-[10px] text-gray-400">Total Mantras</p>
          <p id="japa-total-mantras" class="text-base font-bold text-amber-300 font-mono">0</p>
        </div>
      </div>

      <div class="flex justify-between items-center pt-1">
        <button onclick="resetJapaCounter()" class="text-[10px] text-rose-400 hover:text-rose-300 font-semibold">Reset Round</button>
        <span class="text-[10px] text-gray-400 font-mono">Tap circle to increment</span>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    const coreShlokas = [
      {
        ...
        takeaway: "Threefold qualification of a disciple: praṇipāta (surrender), paripraśna (submissive inquiry), sevā (service)."
      },
      {
        ref: "BG 2.13",
    ...
    function loadUserData() {
      try {
        ...
        const j = localStorage.getItem('vv_japa');
        if (j) {
          const jd = JSON.parse(j);
          japaBeads = jd.beads || 0;
          japaRounds = jd.rounds || 0;
          japaTotalMantras = jd.total || 0;
        }
    ...
    function saveUserData() {
      try {
        ...
        localStorage.setItem('vv_japa', JSON.stringify({ beads: japaBeads, rounds: japaRounds, total: japaTotalMantras }));
        localStorage.setItem('vv_sadhana', JSON.stringify(sadhanaStats));
      } catch (e) {}
      renderDashboard();
    }

    function vibrateHaptic(ms = 15) {
    ...
    function startVoiceSearch() {
      ...
      if (!SpeechRecognition) {
        alert('Voice recognition is not supported on this browser. Try Google Chrome or Android WebView.');
        return;
      }
      const recognition = new SpeechRecognition();
      recognition.lang = 'en-US';
      const btn = document.getElementById('voice-search-btn');
      if (btn) btn.className = 'text-rose-500 animate-pulse text-xs p-1';
      ...
      recognition.onend = () => {
        if (btn) btn.className = 'text-indigo-400 hover:text-indigo-300 text-xs p-1';
      };
      recognition.start();
    }

    // === 🤖 Vani AI Assistant Integration ===
    const GROQ_KEY = 'gsk_' + 'qLsQWnMn9GJ1Kf1gpliDWGdyb3FYW3dcvs7Am0Sycu9l1l0IZhfZ';

    function openVaniAIModal() {
      vibrateHaptic();
      document.getElementById('vani-ai-modal').classList.remove('hidden');
    }

    function closeVaniAIModal() {
      document.getElementById('vani-ai-modal').classList.add('hidden');
    }

    function askAIPreset(q) {
      document.getElementById('ai-user-input').value = q;
      sendAIMessage();
    }
    ...
    async function sendAIMessage() {
      ...
      log.innerHTML += `
        <div class="p-2.5 rounded-xl bg-[#1c2234] border border-[#2b354d] text-gray-100 leading-relaxed text-right ml-6">
          <span class="font-bold text-indigo-400 block pb-0.5 text-[10px]">You</span>
          ${q}
        </div>
        <div id="ai-loading-bubble" class="p-2.5 rounded-xl bg-[#161a28] border border-[#262f44] text-amber-300 mr-6">
          <i class="fa-solid fa-spinner fa-spin mr-1.5"></i> Vani AI is reflecting on Vedic purports...
        </div>
      `;
      ...
      try {
        const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are Vani AI, a Vedic philosophy scholar and study assistant dedicated to the discourses of Dr. Laxmidhar Behera (Director IIT Mandi, HG Lila Purushottam Das) and the Bhaktivedanta purports of A.C. Bhaktivedanta Swami Prabhupada. Provide concise, highly philosophical, uplifting, and clear answers citing Bhagavad Gita, Sankhya, and Srimad Bhagavatam.'
              },
              { role: 'user', content: q }
            ],
            max_tokens: 350
        ...
        if (res.ok) {
          ...
          answer = data.choices[0]?.message?.content || 'Hare Krishna! Always remember Krishna and never forget Him.';
        } else {
          throw new Error('Fallback response');
        }
      } catch (e) {
        answer = `Hare Krishna! According to Dr. Laxmidhar Behera Sir's lectures on Bhagavad Gita and Sankhya philosophy, the material creation consists of 24 elements (Prakriti), while the conscious observer (Atman) is the 25th element, eternally distinct and sustained by Krishna (Paramatman). Chanting and hearing (Sravanam & Kirtanam) directly awaken this constitutional identity.`;
      }
      ...
      log.innerHTML += `
        <div class="p-2.5 rounded-xl bg-[#161a28] border border-[#262f44] text-gray-200 leading-relaxed mr-6">
          <span class="font-bold text-amber-300 block pb-1 text-[10px]"><i class="fa-solid fa-robot"></i> Vani AI</span>
          ${answer.replace(/\n/g, '<br/>')}
        </div>
      `;
      log.scrollTop = log.scrollHeight;
    ...
    function openJapaModal() {
      vibrateHaptic();
      updateJapaUI();
      document.getElementById('japa-modal').classList.remove('hidden');
    }

    function closeJapaModal() {
      document.getElementById('japa-modal').classList.add('hidden');
    }
    ...
    function tapJapaBead() {
      ...
      if (japaBeads >= 108) {
        japaBeads = 0;
        japaRounds++;
        alert(`🎉 Haribol! Completed Round ${japaRounds} of Japa!`);
      }
      updateJapaUI();
      saveUserData();
    }
    ...
    function resetJapaCounter() {
      vibrateHaptic();
      if (!confirm('Reset current bead counter to 0?')) return;
      japaBeads = 0;
      updateJapaUI();
      saveUserData();
    }
    ...
    function updateJapaUI() {
      document.getElementById('japa-bead-count').textContent = japaBeads;
      document.getElementById('japa-round-count').textContent = `${japaRounds} / 16`;
      document.getElementById('japa-total-mantras').textContent = japaTotalMantras;
